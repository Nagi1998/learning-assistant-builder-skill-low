...
最终项目不再允许以题目问答、连续做题或答题判分为核心。学习目标必须通过探索、操控、实验、建造、创作、收集、角色扮演、策略决策或情境挑战等游戏化方式呈现，并形成适合课堂展示的可视化成果；这一限制不增加新的问答轮次或确认节点。

问答环节不再单独追问使用场景、页面布局和 MVP 范围；这些内容会根据学生想法和主流学习助手方向自动推导进设计文档。

每个当前问题可以附带 2-3 个具备创新性的参考灵感，但会明确鼓励学员自主思考，不把参考方向变成必须选择的固定选项；参考灵感不会放在用户回答后的反馈里和下一个问题混在一起。

生成项目代码时会严格贯彻五大高维设计原则：空间与呼吸感、视觉层级、色彩管理、认知减负、情感化与品牌融入；必要时允许轻量技术栈升级，但不能明显拉长生成时间。
